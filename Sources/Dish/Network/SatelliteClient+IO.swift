...
extension SatelliteClient {
    ...
    func sendEncrypted(msgType: UInt16, payload: [UInt8]) {
        ...
        if payloadLen > 0 {
            for i in 0..<Int(payloadLen) { inner[4 + i] = payload[i] }
        }
        ...
        guard let sealed = try? ChaChaPoly.seal(Data(inner),
        ...
        packet.withUnsafeBytes { raw in
            ...
            _ = withUnsafePointer(to: &destCopy) { ptr in
                ptr.withMemoryRebound(to: sockaddr.self, capacity: 1) { sa in
                    Darwin.sendto(sfd, raw.baseAddress, raw.count, 0,
    ...
    func stopHeartbeat() { heartbeatRunning = false }
    ...
    private func receiveOne() {
        ...
        if n < 8 { return }
        // Token check.
        for i in 0..<4 where buf[i] != token[i] { return }
        ...
        guard let sealed = try? ChaChaPoly.SealedBox(nonce: nonce, ciphertext: cipher, tag: tag),
              let plain = try? ChaChaPoly.open(sealed, using: self.currentKey,
                                               authenticating: Data(self.token))
        else { return }
